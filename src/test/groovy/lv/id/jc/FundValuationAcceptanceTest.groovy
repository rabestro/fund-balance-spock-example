...
import lv.id.jc.worker.JuniorWorker
import lv.id.jc.worker.Payment
import lv.id.jc.worker.Performance
import lv.id.jc.worker.SeniorWorker
import lv.id.jc.workscope.Surface
import lv.id.jc.workscope.ZoneFactory
...
class FundValuationAcceptanceTest extends Specification {
    ...
    def "should calculate balance when one worker with one wall assignment #comment"() {
        ...
        Surface.rectangle(9, 5)  | 30           | 450      | 'works two days but less amount then max per day'
        Surface.rectangle(11, 5) | 30           | 782.5    | 'works two days and more amount then max per day'
        Surface.rectangle(11, 5) | 60           | 1082.5   | 'works one day but more amount then max per day'
    }
